;==============================================================
; YOUTUBE VIEWER PRO v6.0 - TURBO (1000 Views, Sem Pop-up, Anti-Crash)
;==============================================================
...
global Parar := false
global QtdVideos := 1000    ; Meta padrão
global MinSeg := 45
global MaxSeg := 120
global VideosOK := 0
global TempoTotal := 0
...
global TrocarCada := 1      ; Troca a cada 1 vídeo para distribuir melhor
global Proxies := []
global ProxyScores := []    ; Pontuação de sucesso de cada proxy
global QtdProxies := 0
global PastaExt := ""
global EdgeUserData := A_Temp . "\YtbViewer_Edge_Data" ; Perfil isolado para não fechar seu Edge pessoal
...
Gui, Add, Text, x315 y145 w100 h20, Proxies Carregados:
Gui, Add, Text, x435 y145 w80 h20 vStatusProxies cLime, 0
Gui, Add, Text, x315 y175 w100 h20, Proxy Atual:
Gui, Add, Text, x315 y195 w260 h20 vProxyInfo cYellow, Nenhum
Gui, Add, Button, x315 y225 w120 h30 gCarregarProxies, Carregar
Gui, Add, CheckBox, x315 y260 w260 h20 vUsarProxy Checked, Usar proxies (Recomendado)
...
Gui, Add, GroupBox, x15 y335 w380 h120 cCyan, Log (Automático)
Gui, Add, Edit, x25 y355 w360 h90 vLogEdit ReadOnly
Gui, Add, GroupBox, x405 y335 w180 h120 cCyan, Status Geral
Gui, Add, Text, x415 y355 w160 h20, Views Concluidas:
Gui, Add, Text, x415 y375 w160 h25 vStatusVids cLime, 0 / 1000
Gui, Add, Text, x415 y405 w160 h20, Tempo Assistido:
...
Log(M) {
    ...
    ; Limita o tamanho do log para não travar a RAM após horas de uso
    if (StrLen(L) > 3000)
        L := SubStr(L, -2000) 
...
CarregarProxies() {
    ...
    Loop, Read, %File%
    {
        ...
        if (P.Length() >= 4) {
            ...
            ProxyScores.Push(0) ; Inicia com score 0
            QtdProxies++
        }
    }
    if (QtdProxies > 0) {
        ...
        Log("Nenhum proxy valido encontrado no txt.")
    }
}

; Escolhe o proxy com maior score (mais sucessos)
ProximoProxy() {
...
Iniciar:
    ...
    if (UsarProxy and QtdProxies = 0) {
        MsgBox, 4, Sem proxies, Continuar sem proxy? (Nao recomendado)
        IfMsgBox, No
            return
        UsarProxy := false
    }
    ...
    Log("========================================")
    Att()
    SetTimer, Rodar, -100 ; Inicia a rotina principal
return
...
Parar:
    ...
    ; Fecha instâncias antigas caso existam
...
Rodar:
    ...
    Loop, %QtdVideos%
    {
        ...
        ; SISTEMA DE RETRY: Se o proxy cair, tenta até 3x antes de pular o vídeo
        While (!VideoConcluido && TentativasVideo < 3) {
            TentativasVideo++
            
            ; Fechar instâncias antigas antes de abrir nova
            Process, Close, msedge.exe
            Sleep, 1500
            ...
            if (UsarProxy and QtdProxies > 0) {
                ...
                P := Proxies[ProxyIdx]
                Log("Video " . N ": Usando Proxy [" . TentativasVideo "/3] " . P.IP . ":" . P.Port)
                GuiControl,, ProxyInfo, % P.IP ":" P.Port " (Score: " . ProxyScores[ProxyIdx] ")"
                Ext := CriarExt(P)
            } else {
                Log("Video " . N ": Sem proxy [" . TentativasVideo "/3]")
            }
            
            ; Abrir Edge de forma ISOLADA
            PID := 0
            ...
            Sleep, 8000 ; Espera carregar a página e o proxy
            
            ; Verifica se o Edge está aberto
            Process, Exist, %PID%
            if (ErrorLevel = 0) {
                Log("ERRO: Navegador fechou sozinho (Proxy bloqueado?).")
                if (ProxyIdx > 0)
                    ProxyScores[ProxyIdx] -= 2 ; Penaliza forte quem falha logo de cara
                continue ; Volta no While para tentar de novo
            }
            
            ; Tempo aleatório de visualização
            Random, T, MinSeg, MaxSeg
            Log("Assistindo por " . T . " segundos...")
            
            ; CRONÔMETRO E MONITORAMENTO
            Pass := 0
            Loop, %T%
            {
                if (Parar or !Rodando)
                ...
                Process, Exist, %PID%
                if (ErrorLevel = 0) {
                    Log("Navegador morreu no meio! Proxy ruim.")
                    if (ProxyIdx > 0)
                        ProxyScores[ProxyIdx] -= 1
                    break ; Sai do Loop de tempo, mas não do de Tentativas
                }
            ...
            ; Verifica se deu tempo suficiente para contar a view (70% do tempo)
            if (Pass >= T * 0.7) {
                VideosOK++
                TempoTotal += Pass
                if (ProxyIdx > 0)
                    ProxyScores[ProxyIdx] += 1 ; Recompensa o proxy bom
                VideoConcluido := true
                Log("OK! View contabilizada (" . VideosOK ").")
            } else {
                Log("FALHA: Tempo insuficiente, tentando novamente...")
            }
            
            ; Fecha o Edge pelo PID específico (não mexe nas suas abas pessoais)
            if (PID)
                Process, Close, %PID%
            Sleep, 1000
        }
        ...
        ; Pausa aleatória entre vídeos para parecer mais humano (5 a 15 segundos)
        if (N < QtdVideos and !Parar and Rodando and VideoConcluido) {
            Random, Pausa, 5000, 15000
            Log("Pausa de " . (Pausa//1000) . "s antes do próximo...")
            Sleep, %Pausa%
        } else if (!VideoConcluido) {
            Log("Descartando vídeo " . N . " após 3 falhas.")
        }
    }
    ...
    Log("")
    Log("========================================")
    Log("  TURBO FINALIZADO!")
    Log("  Views Totais: " . VideosOK
    Log("========================================")
    Att()
    MsgBox, 64, Concluido, Motor Turbo parado.`nViews concluidas: %VideosOK%
return
